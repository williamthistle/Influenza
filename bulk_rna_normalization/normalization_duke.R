...
setwd("C:/Users/willi/OneDrive - Princeton University/Influenza Analysis/Total RNA-Seq/")



counts <- fread("STAREstCountFileT_Vaccitech_SampleNum_Corrected.csv", header = T, sep = ",")
row.names <- as.character(counts$target_id)
...
##############################################
iim=match(rownames(counts), genemap$ensembl_gene_id)
notmatched=which(is.na(iim))
if(length(notmatched)>0){
  counts=counts[-notmatched,]
}#if

geneIDs=genemap[iim, "hgnc_symbol", drop=F]
rownames(geneIDs)=genemap$ensembl_gene_id[iim]
...
write.table(countsSymbol, "STAREstCountFileT_Vaccitech_SampleNum_Corrected.processed.csv", quote = FALSE, sep = "\t")
