...
inputFiles <- paste0(base_scATAC_dir, c(D1.id), "/outs/fragments.tsv.gz")
names(inputFiles) <- names(metadata) <- c(paste0("Sample_", 1:6, "_D1"))
metadata <- c(rep("D1", length(D1.id)))
names(metadata) <- c(paste0("Sample_", 1:6, "_D1"))
...
proj <- ArchRProject(
  ...
  outputDirectory = paste0(output_dir, "ArchR_Pre/"),
...
save.image(paste0(output_dir, "atac_after_filtering_doublets_pre.RData"))

# List available matrices in project
getAvailableMatrices(proj)
#------------------------------------------------------------------------------------------------
# add condition metadata (D1) to cells in project
aa<-proj$Sample
...
save.image(paste0(output_dir, "atac_after_lsi_umap_clusters_1_pre.RData"))
...
save.image(paste0(output_dir, "atac_before_gene_integration_matrix_pre.RData"))
...
rm(scRNA)
save.image(paste0(output_dir, "atac_after_gene_integration_matrix_pre.RData"))
...
save.image(paste0(output_dir, "atac_after_lsi_umap_clusters_2_pre.RData"))
...
save.image(paste0(output_dir, "atac_after_cell_type_voting_pre.RData"))
#saveArchRProject(ArchRProj = proj.filtered, paste0(output_dir, "ArchR_Filtered/"))

#Remove the messy clusters
idxPass <- which(proj.filtered$Clusters %in% c("C2", "C3", "C4", "C5", "C6", "C10", "C17", "C26", "C27", "C43"))
...
export.bed(getPeakSet(proj.filtered),con='pre_peaks.bed')
save.image(paste0(output_dir, "atac_final_pre.RData"))
