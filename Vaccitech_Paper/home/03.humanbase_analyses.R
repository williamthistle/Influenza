...
write.table(down_pathway_plot, paste0(sc_humanbase_dir, "HB_innate_upregulated_genes_blood_gsea_top_downregulated_pathways.tsv"), sep = "\t", quote = FALSE)
...
up_pathway_plot_file <- ggplot(data = up_pathway_plot, aes(x = Module, y = Reactome.Pathway, size = Gene.Count, color = Adjusted.P.Value)) +
  ...
  theme_minimal() +
  labs(
    title = "Reactome Pathways for Upregulated Genes from Innate Immune Cells",
    x = "Module",
    y = "Reactome Pathway",
    size = "Count",
...
ggsave(filename = paste0(sc_humanbase_dir, "HB_innate_upregulated_genes_blood_gsea_top_upregulated_pathways.tiff"), plot = up_pathway_plot_file, device='tiff', dpi=300)
...
down_pathway_plot_file <- ggplot(data = down_pathway_plot, aes(x = Module, y = Reactome.Pathway, size = Gene.Count, color = Adjusted.P.Value)) +
  ...
  theme_minimal() +
  labs(
    title = "Reactome Pathways for Downregulated Genes from Innate Immune Cells",
    x = "Module",
    y = "Reactome Pathway",
    size = "Count",
    color = "Adjusted P Value"
...
ggsave(filename = paste0(sc_humanbase_dir, "HB_innate_downregulated_genes_blood_gsea_top_downregulated_pathways.tiff"), plot = down_pathway_plot_file, device='tiff', dpi=300)
