...
for(atac_cell_type in atac_cell_types_for_mintchip_analysis) {
  ...
  for(matching_cell_type in matching_snME_cell_types) {
    ...
    if(nrow(overlap) > 0) {
      cell_type_overlap_df <- combineRows(atac_cell_type_peaks, snME_dms_subset, overlap)
      cell_type_overlap_df$cell_type <- matching_cell_type
      cell_type_marker_overlap_df[[length(cell_type_marker_overlap_df) + 1]] <- cell_type_overlap_df
    }
  }
  cell_type_overlap_df <- do.call(rbind, cell_type_overlap_df)
  colnames(cell_type_overlap_df)[10] <- "seqnames_marker"
  colnames(cell_type_overlap_df)[11] <- "start_marker"
  colnames(cell_type_overlap_df)[12] <- "end_marker"
