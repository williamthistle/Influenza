...
# I wonder if there's overlap between snATAC-seq and snMethylation? Both hg38 so at least it's easy
# snATAC-seq has value, start, end
# methylation has chr, start, end
sc_peaks$chr <- sc_peaks$value

# Initialize an empty list to store the filtered rows
filtered_rows <- list()

# Iterate through rows in data frame B
snME_dms_blah <- snME_dms[snME_dms$celltype == "Monocyte",]
for (i in 1:nrow(snME_dms_blah)) {
  print(i)
  # Extract values from data frame B
  chr_val <- snME_dms_blah[i, "chr"]
  coordinate_val <- snME_dms_blah[i, "start"]
  cell_type <- snME_dms_blah[i, "celltype"]
  
  # Subset data frame A based on 'chr' value
  subset_A <- subset(sc_peaks, chr == chr_val)
  
  # Filter rows based on the condition (coordinate within start and end)
  filtered_row <- subset(subset_A, coordinate_val >= start & coordinate_val <= end)
...
# Combine the filtered rows into a single data frame
final_result_mono <- do.call(rbind, filtered_rows)

# final_result contains peaks that contain DMS
