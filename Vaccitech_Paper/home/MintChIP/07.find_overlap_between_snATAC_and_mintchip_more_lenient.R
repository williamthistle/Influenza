...
unfiltered_all_marker_overlap_nums_more_lenient <- data.frame(marker = character(), num_marker_peaks = numeric(),
...
for(marker in mintchip_markers) {
  ...
  # Increase length of mintchip peaks (more_lenient)
  all_marker_peaks$start <- all_marker_peaks$start - 400
  all_marker_peaks$end <- all_marker_peaks$end + 400 
  all_marker_peaks_granges <- makeGRangesFromDataFrame(df = all_marker_peaks, keep.extra.columns = TRUE)
  all_marker_overlap <- as.data.frame(findOverlaps(all_marker_peaks_granges, sc_peaks_more_lenient_granges))
  marker_percentage_peak_overlap <- nrow(all_marker_overlap) / nrow(all_marker_peaks)
  atac_percentage_peak_overlap <- nrow(all_marker_overlap) / nrow(sc_peaks_more_lenient)

  current_row <- c(marker, nrow(all_marker_peaks), nrow(sc_peaks_more_lenient), nrow(all_marker_overlap),
                   marker_percentage_peak_overlap, atac_percentage_peak_overlap)
  current_row <- data.frame(t(current_row))
  colnames(current_row) <- colnames(unfiltered_all_marker_overlap_nums_more_lenient)
  unfiltered_all_marker_overlap_nums_more_lenient <- rbind(unfiltered_all_marker_overlap_nums_more_lenient, current_row)
}

# The overlap is pretty good for all markers!
unfiltered_all_marker_overlap_nums_more_lenient
...
das_marker_overlap_nums_more_lenient <- data.frame(marker = character(), num_das_marker_peaks = numeric(),
                                                 num_atac_peaks = numeric(), num_overlapping_peaks = numeric(),
...
for(marker in mintchip_markers) {
  ...
  das_marker_with_hg38_df <- add_hg38_coordinates_to_marker_peaks(read.table(paste0(mintchip_das_dir, marker, "/", marker, "_DESeq2_FC_0.tsv"),
                                       ...
                                       read.table(paste0(mintchip_das_dir, marker, "/", marker, "_all_peaks_with_hg38_coordinates.tsv"),
                                                  sep = "\t", header = TRUE))
  # Increase length of mintchip peaks (more_lenient)
  das_marker_with_hg38_df$start <- das_marker_with_hg38_df$start - 400
  das_marker_with_hg38_df$end <- das_marker_with_hg38_df$end + 400 
  ...
  das_marker_overlap <- as.data.frame(findOverlaps(das_marker_peaks_granges, sc_peaks_more_lenient_granges))
  das_marker_overlap_df <- combineRows(das_marker_with_hg38_df, sc_peaks_more_lenient, das_marker_overlap)
  marker_percentage_peak_overlap <- length(unique(das_marker_overlap_df$start)) / nrow(das_marker_with_hg38_df)
  atac_percentage_peak_overlap <- length(unique(das_marker_overlap_df$idx)) / nrow(sc_peaks_more_lenient)
  
  current_row <- c(marker, nrow(das_marker_with_hg38_df), nrow(sc_peaks_more_lenient), length(unique(das_marker_overlap_df$start)),
                   marker_percentage_peak_overlap, atac_percentage_peak_overlap)
  current_row <- data.frame(t(current_row))
  colnames(current_row) <- colnames(das_marker_overlap_nums_more_lenient)
  das_marker_overlap_nums_more_lenient <- rbind(das_marker_overlap_nums_more_lenient, current_row)
}

# Not too bad!
das_marker_overlap_nums_more_lenient
...
atac_mintchip_tables_more_lenient <- list()
for(atac_cell_type in atac_cell_types_for_mintchip_analysis) {
  ...
  atac_cell_type_peaks$start <- as.numeric(start)
  atac_cell_type_peaks$end <- as.numeric(end)
  # Make cell type peaks more more_lenient
  atac_cell_type_peaks$start <- atac_cell_type_peaks$start - 500
  atac_cell_type_peaks$end <- atac_cell_type_peaks$end + 500
  ...
  atac_mintchip_tables_more_lenient[[length(atac_mintchip_tables_more_lenient) + 1]] <- cell_type_marker_overlap_df
}
atac_mintchip_tables_more_lenient <- do.call(rbind, atac_mintchip_tables_more_lenient)

# Summarize overlap for different cell types and different markers
print(table(atac_mintchip_tables_more_lenient$marker))
for(cell_type in unique(atac_mintchip_tables_more_lenient$Cell_Type)) { 
  print(cell_type)
  cell_type_subset <- atac_mintchip_tables_more_lenient[atac_mintchip_tables_more_lenient$Cell_Type == cell_type,]
  print(table(cell_type_subset$marker))
}

# Find MAGICAL overlap
MAGICAL_overlap_with_mintchip_more_lenient <- list()
for(cell_type in unique(atac_mintchip_tables_more_lenient$Cell_Type)) {
  print(cell_type)
  cell_type_for_magical <- sub(" ", "_", cell_type)
  atac_mintchip_tables_cell_type_subset <- atac_mintchip_tables_more_lenient[atac_mintchip_tables_more_lenient$Cell_Type == cell_type,]
  ...
  magical_results_cell_type_subset$start <- magical_results_cell_type_subset$start - 500
  magical_results_cell_type_subset$end <- magical_results_cell_type_subset$end + 500
  result <- merge(magical_results_cell_type_subset, atac_mintchip_tables_cell_type_subset, by = c("seqnames", "start", "end"))
  MAGICAL_overlap_with_mintchip_more_lenient[[length(MAGICAL_overlap_with_mintchip_more_lenient) + 1]] <- result
}
MAGICAL_overlap_with_mintchip_more_lenient <- do.call(rbind, MAGICAL_overlap_with_mintchip_more_lenient)