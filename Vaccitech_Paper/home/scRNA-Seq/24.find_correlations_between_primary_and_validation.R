...
for(cell_type in correlation_cell_types) {
  ...
  sc_correlation_plots[[cell_type]][["sc_primary_pseudobulk_corrected_vs_sc_vaccinated"]] <- ggplot(data = comparing_first_vs_second_df, mapping = aes(x = first_fc, y = second_fc)) +
    geom_point(size = 2) +
    sm_statCorr(corr_method = "spearman", text_size = 6) + xlab("Naive HVL FC") + ylab("Naive vaccinated FC") + labs(title = cell_type_no_underscore) +  xlim(-2.5, 2.5) + ylim(-2.5, 2.5) + theme(aspect.ratio = 1, text=element_text(size=15))
  
  
  # Check correlation of union of HVL and vaccinated pseudobulk corrected genes
  union_sc_pseudobulk_corrected_degs <- union(rownames(cell_type_sc_pseudobulk_corrected_degs), rownames(cell_type_vaccinated_sc_pseudobulk_corrected_degs))
