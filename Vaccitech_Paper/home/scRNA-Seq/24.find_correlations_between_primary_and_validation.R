...
for(cell_type in correlation_cell_types) {
  ...
  cell_type_sc_degs <- read.table(paste0(scRNA_hvl_placebo_deg_dir, "D28-vs-D_minus_1-degs-", cell_type, "-time_point-controlling_for_subject_id_sc.tsv"),
                                  sep = "\t", header = TRUE)
  cell_type_vaccinated_sc_degs <- read.table(paste0(scRNA_hvl_vaccinated_deg_dir, "D28-vs-D_minus_1-degs-", cell_type, "-time_point-controlling_for_subject_id_sc.tsv"),
                                      sep = "\t", header = TRUE)
  ...
  cell_type_vaccinated_sc_pseudobulk_corrected_degs <- read.table(paste0(scRNA_hvl_vaccinated_deg_dir, "D28-vs-D_minus_1-degs-", cell_type, "-time_point-controlling_for_subject_id_final.tsv"),
                                                           sep = "\t", header = TRUE)
  rownames(cell_type_vaccinated_sc_pseudobulk_corrected_degs) <- cell_type_vaccinated_sc_pseudobulk_corrected_degs$Gene_Name
  
  # Check correlation for primary significant SC genes in vaccinated data
  ...
  compare_second_df <- unfiltered_cell_type_vaccinated_sc_degs[rownames(unfiltered_cell_type_vaccinated_sc_degs) %in% primary_sc_degs,]
  ...
  sc_correlations[[cell_type]][["sc_primary_vs_sc_vaccinated"]] <- correlation_val
  
  # Plot correlation
  sc_correlation_plots[[cell_type]][["sc_primary_vs_sc_vaccinated"]] <- ggplot(data = comparing_first_vs_second_df, mapping = aes(x = first_fc, y = second_fc)) +
    geom_point(size = 2) +
    sm_statCorr() + xlab("High Viral Load FC") + ylab("Low Viral Load FC") + labs(title = cell_type_no_underscore)
  
  # Check correlation for primary significant SC genes (pseudobulk_corrected) in vaccinated data
  ...
  compare_second_df <- unfiltered_cell_type_vaccinated_sc_degs[rownames(unfiltered_cell_type_vaccinated_sc_degs) %in% primary_sc_pseudobulk_corrected_degs,]
  compare_first_df <- compare_first_df[rownames(compare_first_df) %in% rownames(compare_second_df),]
  compare_second_df <- compare_second_df[rownames(compare_second_df) %in% rownames(compare_first_df),]
  compare_first_df <- compare_first_df[order(rownames(compare_first_df)),]
  compare_second_df <- compare_second_df[order(rownames(compare_second_df)),]
  ...
  print(correlation_val$estimate)
  print(correlation_val$p.value)
  sc_correlations[[cell_type]][["sc_primary_pseudobulk_corrected_vs_sc_vaccinated"]] <- correlation_val
  
  # Plot correlation
  # Idea! Use data to set limits. 
  
  sc_correlation_plots[[cell_type]][["sc_primary_pseudobulk_corrected_vs_sc_vaccinated"]] <- ggplot(data = comparing_first_vs_second_df, mapping = aes(x = first_fc, y = second_fc)) +
    geom_point(size = 2) +
    sm_statCorr(corr_method = "spearman", text_size = 6) + xlab("Naive HVL FC") + ylab("Vaccinated HVL FC") + labs(title = cell_type_no_underscore) +  xlim(-2.5, 2.5) + ylim(-2.5, 2.5) + theme(aspect.ratio = 1, text=element_text(size=15))
}

hvl_vs_vaccinated_pseudobulk_corrected_plots <- lapply(sc_correlation_plots, function(x) x[[2]])
