# HVL
hvl_sc_degs_table <- data.frame(Cell_Type = character(), Gene = character(), log2FC = numeric())
...
hvl_sc_degs_table <- subset(hvl_sc_degs_table, !(Cell_Type == "HSPC"))
hvl_sc_degs_table <- subset(hvl_sc_degs_table, !(Cell_Type == "NK_CD56bright"))

hvl_sc_degs_table <- hvl_sc_degs_table %>%
...
pos_all_sc_degs <- hvl_sc_degs_table[hvl_sc_degs_table$log2FC > 0,]
neg_all_sc_degs <- hvl_sc_degs_table[hvl_sc_degs_table$log2FC < 0,]



cell_type_order <- c("CD4 Naive", "pDC", "CD8 Naive", "cDC", "MAIT", "CD16 Mono", "B Memory", "B Naive", "CD4 Memory", "CD14 Mono", "NK", "CD8 Memory")
hvl_sc_degs_table$Cell_Type <- factor(hvl_sc_degs_table$Cell_Type, levels = cell_type_order)
hvl_sc_degs_table$direction <- factor(hvl_sc_degs_table$direction, levels = c("Upregulated", "Downregulated"))

all_sc_degs_plot <- ggplot(hvl_sc_degs_table, aes(Cell_Type, log2FC, color = direction)) +
