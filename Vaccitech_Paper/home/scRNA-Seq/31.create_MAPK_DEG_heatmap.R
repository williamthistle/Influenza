...
heatmap_genes <- c("MAPK7", "MAPK8", "MAP2K1", "MAP3K2", "MAP3K8", "MAP3K11", "MAP3K20", "MAP4K3", "MAPKAPK2")
...
mapk_heatmap_df <- data.frame(Cell_Type = cell_type_vector, Gene_Name = gene_name_vector, fold_change = fold_change_vector, significant = significance_vector)

mapk_heatmap_df$Gene_Name <- factor(mapk_heatmap_df$Gene_Name, levels = unique(mapk_heatmap_df$Gene_Name))
mapk_heatmap_df$Cell_Type <- factor(mapk_heatmap_df$Cell_Type, levels = c("CD14 Mono", "CD16 Mono", "NK", "NK_CD56bright", "cDC", "pDC"))


ggplot() + 
  geom_raster(data = mapk_heatmap_df, aes(x = Cell_Type, y = Gene_Name, fill = fold_change)) +
  geom_text(data = mapk_heatmap_df, aes(x = Cell_Type, y = Gene_Name, label = significant), nudge_y = 0.15, nudge_x = 0.40, size = 6) + scale_fill_gradient2(low="navy", mid="white", high="red") +
  theme_classic() + labs(title = "Fold Change for MAPK-Related DEG in Innate Immune Cell Types",
                      x = "Cell Type",
