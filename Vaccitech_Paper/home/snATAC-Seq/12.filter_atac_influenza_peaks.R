...
innate_snATAC_cell_types <- c("CD14 Mono", "CD16 Mono", "NK", "cDC", "pDC")
promoter_terms <- c("Promoter (<=1kb)", "Promoter (1-2kb)", "Promoter (2-3kb)")
das_dirs <- c(scATAC_hvl_placebo_das_dir, scATAC_hvl_vaccinated_das_dir, scATAC_lvl_placebo_das_dir)
...
for(das_dir in das_dirs) {
  for(snATAC_cell_type in innate_snATAC_cell_types) {
    ...
    for(pct in c(0.01, 0.05, 0.1)) {
      sc_differential_analysis_results_file <- paste0(das_dir, "D28-vs-D_minus_1-degs-", snATAC_cell_type_for_file_name, "-time_point-controlling_for_subject_id_sc_pct_", pct, ".tsv")
      sc_differential_analysis_results <- read.table(sc_differential_analysis_results_file, sep = "\t", header = TRUE)
      final_differential_analysis_results_file <- paste0(das_dir, "D28-vs-D_minus_1-degs-", snATAC_cell_type_for_file_name, "-time_point-controlling_for_subject_id_final_pct_", pct, ".tsv")
...
for(das_dir in das_dirs) {
  for(snATAC_cell_type in innate_snATAC_cell_types) {
    ...
    for(analysis_type in c("sc", "final")) {
      for(pct in c(0.01, 0.05, 0.1)) {
        differential_analysis_results_file <- paste0(das_dir, "D28-vs-D_minus_1-degs-", snATAC_cell_type_for_file_name, "-time_point-controlling_for_subject_id_", analysis_type, "_pct_", pct, ".tsv")
...
for(das_dir in das_dirs) {
  snATAC_peak_annotated_dir <- paste0(das_dir, "annotated/")
  if (!dir.exists(snATAC_peak_annotated_dir)) {dir.create(snATAC_peak_annotated_dir)}
  for(snATAC_cell_type in innate_snATAC_cell_types) {
    snATAC_cell_type_for_file_name <- sub(" ", "_", snATAC_cell_type)
    # Read in full list of unfiltered SC peaks
    differential_analysis_results_file <- paste0(das_dir, "D28-vs-D_minus_1-degs-", snATAC_cell_type_for_file_name, "-time_point-controlling_for_subject_id_sc_unfiltered.tsv")
    differential_analysis_results <- read.table(differential_analysis_results_file, sep = "\t", header = TRUE)
