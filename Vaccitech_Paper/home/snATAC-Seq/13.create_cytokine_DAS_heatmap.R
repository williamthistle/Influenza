...
for(current_row in 1:nrow(cytokine_das_coords)) {
  ...
  current_cytokine_type <- current_coords$cytokine_type
  for(innate_cell_type in innate_cell_types_atac) {
    ...
    cytokine_type_vector <- c(cytokine_type_vector, current_cytokine_type)
...
cytokine_das_heatmap_df <- data.frame(Cell_Type = cell_type_vector, Gene_Name = gene_name_vector, chr = chr_vector,
                                  ...
                                  significant = significance_vector, cytokine_type = cytokine_type_vector)
...
ggsave("C:/Users/wat2/Desktop/cytokine_barplot_without_legend.png", plot = patchwork::wrap_plots(cytokine_das_plots, ncol = 2, nrow = 2), height = 10, width = 14)
