...
for(cell_type in atac_cell_types) {
  ...
  full_sc_das <- read.table(paste0(sc_das_dir, "diff_peaks/D28-vs-D_minus_1-degs-", cell_type_for_file_name, "-time_point-controlling_for_subject_id_overlapping_peak_pct_0.01.tsv"),
                                    sep = "\t", header = TRUE)
...
cell_type_order <- c("CD4 Memory", "Proliferating", "MAIT", "T Naive", "CD16 Mono", "CD8 Memory", "B", "NK", "CD14 Mono")
...
all_sc_das_plot <- ggplot(all_sc_das_pseudobulk_corrected_table, aes(Cell_Type, log2FC, color = direction)) +
  ...
  ylab("log2FC (Site Expression)") + ggtitle("scATAC DASs by Cell Type (28 Days Post-Exposure vs Pre-Exposure)") +
