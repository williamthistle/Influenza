...
for(cell_type in atac_cell_types) {
  ...
  full_sc_das <- read.table(paste0(scATAC_hvl_placebo_das_dir, "D28-vs-D_minus_1-degs-", cell_type_for_file_name, "-time_point-controlling_for_subject_id_overlapping_peak_pct_0.01.tsv"),
                                    sep = "\t", header = TRUE)
...
cell_type_order <- c("CD4 Memory", "Proliferating", "MAIT", "CD4 Naive", "CD8 Naive", "cDC", "pDC", "CD16 Mono", "CD8 Memory", "B", "NK", "CD14 Mono")
