...
lvl_magical_df <- read.table(paste0(MAGICAL_lvl_placebo_output_dir, "MAGICAL_overall_output_with_stage_1_info.tsv"), sep = "\t", header = TRUE)
...
for(cell_type in correlation_cell_types) {
  ...
  lvl_magical_peak_set <- lvl_magical_df[lvl_magical_df$Cell_Type == cell_type,]
  lvl_magical_peak_set <- paste0(lvl_magical_peak_set$Peak_chr, "-", lvl_magical_peak_set$Peak_start, "-", lvl_magical_peak_set$Peak_end)
  cell_type_validation_sc_das <- unfiltered_cell_type_validation_sc_das[rownames(unfiltered_cell_type_validation_sc_das) %in% lvl_magical_peak_set,]
...
hvl_vs_lvl_pseudobulk_corrected_plots <- lapply(sc_correlation_plots, function(x) x[[1]])
ggsave("C:/Users/willi/Desktop/hvl_vs_lvl_fc_magical_das_correlation.png", plot = patchwork::wrap_plots(hvl_vs_lvl_pseudobulk_corrected_plots, ncol = 2, nrow = 2), height = 10, width = 10)
