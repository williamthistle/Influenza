...
# magical_dirs <- c(MAGICAL_hvl_placebo_dir, MAGICAL_hvl_vaccinated_dir, MAGICAL_lvl_placebo_dir)
magical_dirs <- c(MAGICAL_hvl_vaccinated_dir, MAGICAL_lvl_placebo_dir)

for(magical_dir in magical_dirs) {
  # STEP 0: Set up directories and global files
  cell_type_candidate_gene_dir <- paste0(magical_dir, "Candidate_Genes/")
  if (!dir.exists(cell_type_candidate_gene_dir)) {dir.create(cell_type_candidate_gene_dir, recursive = TRUE)}
  cell_type_candidate_peak_dir <- paste0(magical_dir, "Candidate_Peaks/")
  if (!dir.exists(cell_type_candidate_peak_dir)) {dir.create(cell_type_candidate_peak_dir, recursive = TRUE)}
  cell_type_scATAC_cell_metadata_dir <- paste0(magical_dir, "scATAC_Cell_Metadata/")
  if (!dir.exists(cell_type_scATAC_cell_metadata_dir)) {dir.create(cell_type_scATAC_cell_metadata_dir, recursive = TRUE)}
  cell_type_scRNA_cell_metadata_dir <- paste0(magical_dir, "scRNA_Cell_Metadata/")
  if (!dir.exists(cell_type_scRNA_cell_metadata_dir)) {dir.create(cell_type_scRNA_cell_metadata_dir, recursive = TRUE)}
  cell_type_scATAC_read_counts_dir <- paste0(magical_dir, "scATAC_Read_Counts/")
  if (!dir.exists(cell_type_scATAC_read_counts_dir)) {dir.create(cell_type_scATAC_read_counts_dir, recursive = TRUE)}
  cell_type_scRNA_read_counts_dir <- paste0(magical_dir, "scRNA_Read_Counts/")
  if (!dir.exists(cell_type_scRNA_read_counts_dir)) {dir.create(cell_type_scRNA_read_counts_dir, recursive = TRUE)}
  magical_output_dir <- paste0(magical_dir, "Output/")
  if (!dir.exists(magical_output_dir)) {dir.create(magical_output_dir, recursive = TRUE)}
  
  magical_input_scRNA_dir <- paste0(magical_dir, "Original_scRNA_Data/")
  magical_input_scATAC_dir <- paste0(magical_dir, "Original_scATAC_Data/")
  
  hg38_ref_seq <- paste0(magical_dir, "hg38_Refseq.txt")
  peak_coordinates <- paste0(magical_dir, "scATAC_Peak_Coordinates/ATAC_peak_coordinates.tsv")
  gene_list <- paste0(magical_dir, "scRNA_Genes/RNA_genes.tsv")
  motifs <- paste0(magical_dir, "Motifs.txt")
  motif_prior <- paste0(magical_dir, "scATAC_Motif_Mapping_Prior/ATAC_motif_mapping_prior.tsv")
  tads <- paste0(magical_dir, "RaoGM12878_40kb_TopDomTADs_filtered_hg38.txt")
  ...
  for(cell_type in cell_types) {
    current_deg <- read.table(paste0(magical_input_scRNA_dir, "D28-vs-D_minus_1-degs-", sub(" ", "_", cell_type), "-time_point-controlling_for_subject_id_final.tsv"), sep = "\t",
                              header = TRUE)
    write.table(current_deg$Gene_Name,
  ...
  for(cell_type in cell_types) {
    current_das <- read.table(paste0(magical_input_scATAC_dir, "D28-vs-D_minus_1-degs-", sub(" ", "_", cell_type), "-time_point-controlling_for_subject_id_sc_pct_0.01.tsv"), sep = "\t",
                              header = TRUE)
  ...
  for(cell_type in cell_types) {
    ...
    # Add HVL back in ATAC if we re-process HVL placebo (or fix file names so that this token is removed)
    current_scRNA_cell_metadata <- paste0(cell_type_scRNA_cell_metadata_dir, cell_type, "_HVL_RNA_cell_metadata.tsv")
    current_scATAC_cell_metadata <- paste0(cell_type_scATAC_cell_metadata_dir, cell_type, "_ATAC_cell_metadata.tsv")
    
    current_scRNA_read_counts <- paste0(cell_type_scRNA_read_counts_dir, cell_type, "_HVL_RNA_read_counts.rds")
    current_scATAC_read_counts <- paste0(cell_type_scATAC_read_counts_dir, cell_type, "_ATAC_read_counts.rds")
