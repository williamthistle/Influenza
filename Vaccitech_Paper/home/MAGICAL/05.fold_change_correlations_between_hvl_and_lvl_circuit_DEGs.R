...
for(cell_type in correlation_cell_types) {
  ...
  cell_type_lvl_magical_df <- lvl_magical_df[lvl_magical_df$Cell_Type == cell_type,]
  cell_type_lvl_magical_genes <- unique(cell_type_lvl_magical_df$Gene_symbol)
  
  # Check correlation of union of HVL and LVL pseudobulk corrected genes
  union_sc_pseudobulk_corrected_degs <- union(cell_type_hvl_magical_genes, cell_type_lvl_magical_genes)
  ...
  if(nrow(comparing_first_vs_second_df) > 10) {
...
ggsave("C:/Users/willi/Desktop/hvl_vs_lvl_fc_magical_genes_correlation.png", plot = patchwork::wrap_plots(hvl_vs_lvl_pseudobulk_corrected_plots, ncol = 2, nrow = 2), height = 10, width = 10)
